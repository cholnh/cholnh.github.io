...
반복적인 코드를 줄이는 것은 코드 생산성에 큰 영향을 미칠 뿐만 아니라 관리 포인트를 줄여 유지보수에도 용이합니다.  

자바에서는 이러한 지루하고 반복적인 코드를 줄이기 위한 여러 도구들을 제공합니다.  
그 중 하나인 **Annotation Processor** 를 사용한다면 컴파일 시점에 코드를 자동으로 생성하는 기능을 활용할 수 있습니다.   
...
> 실제 com.sun.tools API 설명에도 아래와 같이 '**예고없이 변경될 수 있다**' 라고 경고하고 있습니다.
...
아래 그림을 통해 기존 코드가 어떻게 AST 로 파싱되어 있는지 알 수 있습니다.
...
META-INF 내 Processor 파일을 정의하는 방식과 달리 google 에서 제공하는 [auto-service](https://mvnrepository.com/artifact/com.google.auto.service/auto-service/1.0.1) 라이브러리를 이용하면 어노테이션 하나로 등록이 완료됩니다.
...
```
  ...
  + 두 어노테이션 대신 `AbstractProcessor` 내 `getSupportedAnnotationTypes()` `getSupportedSourceVersion()` 재정의 하여도 됩니다.
...
```
  ...
  + `processingEnv` 는 부모인 `AbstractProcessor` 에서 제공됩니다. 해당 객체를 통해 `com.sun.source` 에서 제공되는 인스턴스들을 생성할 수 있습니다.

<br/>

#### 만약 com.sun.source.* 모듈이 import 되지 않는다면?

해당 모듈 gradle 내 compileJava 를 아래와 같이 추가합니다.
(만약 사용할 모듈이 더 있다면 아래 exports 구문을 사용하여 추가합니다)
...
> 주의:   
> 위 gradle 에서 적은 옵션과 문법이 살짝 다릅니다.

<br/>

다시 본론으로 돌아와 Scanner 를 정의합니다.  
`TreePathScanner` 에서는 AST 를 순회하며 원하는 위치의 코드를 접근할 수 있게 합니다.
...
public class StaticFactoryTreePathScanner extends TreePathScanner<Object, CompilationUnitTree> {
    ...
    public StaticFactoryTreePathScanner(StaticFactory annotation, ProcessingEnvironment processingEnv) {
        final var treeTranslator = StaticFactoryClassTreeTranslator.of(annotation, processingEnv);
...
```
  ...
  + `TreeMaker.Block()` : 코드 블럭을 의미합니다. 파라미터에서 값을 받아 멤버 변수에 assign 시켜주는 로직은 아래에 기술하겠습니다. body 에 넣는 코드 블럭인 경우 첫 번째 파라미터인 flags 는 0으로 지정합니다. (플래그를 사용하지 않음을 나타냅니다)  
...
위에서 생성한 private 생성자를 통해 내부에서만 객체가 생성되도록 막아두었으니 static 한 factory 메서드를 통해 객체 생성을 열어줍니다.
...
우선 위에서 만든 모듈을 사용할 호출 모듈을 만들어야 합니다. 적당한 모듈을 만든 뒤 gradle 의존성을 추가해줍니다.
...
이전에 gradle 에 지정해둔 `compileJava` 를 실행하면 컴파일러가 해당 옵션을 적용하여 `com.sun.tools` 모듈을 사용하여 Annotation Processor 를 실행합니다.
...
AST 를 수정하여 코드를 조작하는 방법이 탐탁치 않다면 JavaPoet 등의 OpenApi 를 사용하여 파일 생성(수정이 아닙니다)을 하여도 됩니다.
반복적인 코드를 최대한 줄이고 관리 포인트를 하나로 집약하여 생산성과 유지보수 용이성을 높이는 것에 초점을 두었습니다.  

위 기능들을 활용한다면 반복 코드 생성 뿐만 아니라 타입 또는 파라미터 등에 대해 유효성 검사를 추가 하여 코드 안정성을 높일 수도 있을거라 기대합니다.
이 모든걸 컴파일 시점에 성능 누락 하나 없이 말이죠.
